...
try{
    ...
    $pages_vaisseaux = $sql->prepare("SELECT * FROM `vaisseaux` WHERE vaisseau_id = :vaisseau_id");
    // je lui donne les paramètres dont elle a besoin sans en oublier
    $pages_vaisseaux->bindValue(":vaisseau_id", $_GET["vaisseau_id"]);
    // j'éxécute
    $pages_vaisseaux->execute();
...
foreach($pages_vaisseaux as $page_vaisseau){
    ...
    echo '
        <div id="article-text">
        <h1 id="article-title">
        '.$page_vaisseau['vaisseau_nom'].'
        </h1>
        <div id="article-float-right">
        <img id="article-img" src="'.$page_vaisseau['vaisseau_img'].'">
        <ul>
        <li><b>Nom</b> : '.$page_vaisseau['vaisseau_nom'].'</li>
        </ul>
        </div>
            <p id="edition">'.$page_vaisseau['vaisseau_bio'].'</p>
        </div>';
}
